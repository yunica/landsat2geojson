...
def download_feature(feature_, bands_, data_folder_):
    ...
    def download_tiff(file_url_, file_path_, session_):
        status = {
            'is_download': True,
            'file_url': file_url_,
    ...
    status_download = Parallel(n_jobs=-1)(
        delayed(download_tiff)(f"{path_url}/{id}_{band}.TIF", f'{data_folder_band}/{id}_{band}.TIF', session)
        for band in bands_
    )
...
def download_features(features, bands, data_folder):
    status_download = [download_feature(feature, bands, data_folder) for feature in
                       tqdm(features, desc="download features tiff")]
