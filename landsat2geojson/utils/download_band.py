...
def download_scene(feature_, bands_, data_folder_):
    ...
    status_download = Parallel(n_jobs=-1)(
        delayed(download_tiff)(
            f"{path_url}/{id}_{band}.TIF",
            f"{data_folder_band}/{id}_{band}.TIF",
            band,
            session,
        )
        for band in bands_
...
def download_scenes(scenes, bands, data_folder):
    status_download = [
        download_scene(feature, bands, data_folder)
        for feature in tqdm(scenes, desc="download features tiff")
    ]
